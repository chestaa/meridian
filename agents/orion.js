...
function buildSystemPrompt() {
  ...
  const bluechipBlock = [
    ...
    "- Narrative, smart wallets, top10/bundlers, hype, organic score, and the minTokenFeesSol floor are memecoin risk signals and are IRRELEVANT to a stable bluechip. Do NOT skip on them.",
    `- DEEP TVL (>= $${bcMinTvl.toLocaleString("en-US")}) is the EDGE here, not a red flag — depth lets a tight peg range capture steady fees. There is NO maximum-TVL cap for a bluechip: the memecoin max-TVL ceiling (e.g. $150K) and the 'TVL band' criterion do NOT apply — multi-million-dollar TVL is EXPECTED and GOOD. The profit-share micro/thin tier still applies (our take must be non-trivial), but NEVER skip a bluechip for 'TVL too high' / 'above the max-TVL cap' / 'outside the TVL band'.`,
    "- MCAP: a bluechip base leg (SOL/USDC/USDT/cbBTC/staked-SOL) has a market cap in the BILLIONS. The memecoin mcap band (150k-10M) and the 'mcap band' criterion do NOT apply to a bluechip — a large base-asset mcap CONFIRMS a genuine large-cap and is a GREEN flag, never a skip reason. There is NO maximum mcap for a bluechip.",
