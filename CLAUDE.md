...
| broadDiscoveryEnabled | screening | true |
| broadDiscoveryPageSize | screening | 1000 |
| broadMcapFloor / broadMcapCeil | screening | 10k / 50M |
| broadMinTvl | screening | 1k |
| broadSortBy | screening | "fee_active_tvl_ratio:desc" |
| marketRegimeGateEnabled | screening | true |
| regimeDowntrendThresholdPct | screening | -5 |
| regimeUptrendThresholdPct | screening | 5 |
...
**Bluechip LST-SOL deployability guard (Cassiopeia, Opsi 1 pivot — wSOL MUST be tokenY/quote):** in `getTopCandidates()` bluechip block (fires only when `bluechipModeEnabled` — currently PAUSED/OFF — AND `requireBluechipWsolLeg`, default true). Vega diagnosis: Opsi B seeds with SOL only, so SOL can only be the deposit side when wSOL is the pool's **tokenY (QUOTE) leg**. True for memecoin pools and for genuine LST-SOL pools (mint sort → LST=tokenX, wSOL=tokenY: verified on-chain JitoSOL-SOL `BoeMUkCLHchTD31HdXsbDExuZZfcUppSLpYtV3LZTH6U` $2.44M TVL, JupSOL-SOL, mSOL-SOL). FALSE for SOL-USDC (SOL=tokenX/base, USDC=quote) → on-chain `0x1`. The old `bluechipHasWsolLeg` ("either leg is wSOL") was TOO LOOSE — it let SOL-USDC + SOL-mSOL slip to deploy. Tightened to `bluechipWsolQuoteRejectReason(pool)` (pure, exported): deployable ONLY when `tokenY === wSOL`; wSOL-as-base and no-wSOL-leg (JLP-USDC) both rejected. FAIL-CLOSED (anti-pattern #2): missing/null/empty tokenY → reject. Reject reason: `bluechip_wsol_not_quote_side`. **Money-path defense-in-depth (Vega TODO):** `dlmm.js` deploy reads `quoteMint = pool.lbPair.tokenYMint` but does not yet assert `=== wSOL` before depositing — the screener guard fires pre-deploy so no live hole, but a belt-and-suspenders chain-side check belongs in Vega's path. Tests: `scripts/test-bluechip-lstsol-guard.js` (19 assertions).

**Broad Discovery — server→client gate migration (Cassiopeia, CROWN JEWEL):** the root-cause fix for "cuma main 5 pool dari jutaan." `discoverPools()` USED to push EVERY strict gate (organic≥60, fee/TVL, binStep, mcap band, holders, age, ...) to the Pool-Discovery server as `filter_by` params, so the server cut the 114k-pool universe to ~3 BEFORE we saw a candidate. `page_size` was 50 (API ceiling 1000 — pagination is broken, page_size is the only breadth lever). **Fix (NOT a loosening):** `buildDiscoveryFilters(s)` (pure, exported) now returns a WIDE server pre-filter when `broadDiscoveryEnabled` (default true) — only `pool_type=dlmm` + critical-warning/single-ownership sanity flags + a WIDE mcap band (`broadMcapFloor` 10k … `broadMcapCeil` 50M) + low tvl floor (`broadMinTvl` 1k) — plus a free server pre-sort (`broadSortBy` = `fee_active_tvl_ratio:desc`, pulls highest fee/TVL first if the page clips) at `page_size=broadDiscoveryPageSize` (1000). The **IDENTICAL** strict gate then runs CLIENT-side via `getRawPoolScreeningRejectReason` over the broad set — only the evaluation LOCATION moved. **INVARIANT:** every broad bound is wider-or-equal to the matching strict threshold (`broadMcapFloor ≤ minMcap`, `broadMcapCeil ≥ maxMcap`, `broadMinTvl ≤ minTvl`), so the server can NEVER drop a pool the strict client gate would pass — the broad result is a strict SUPERSET of the deployable set. **Cost-control (anti-pattern #8 + Lyra-aware):** enrich-before-gate passes are probe-gated against the STRICT thresholds (a pool failing strict mcap/organic/fee-TVL is dropped from the enrich set for free, no API spent); `getTopCandidates()` deterministically pre-ranks by `scoreCandidate` then HARD-slices to `limit` BEFORE any per-pool enrichment (PVP/Jupiter audit/OKX) and before the judge → **enrichment + judge cost stay FLAT** (still `limit` pools, now the cream of the whole gate-passed universe instead of the cream of 50). `discoverPools()` `returnLimit` caps the RETURNED set for the direct `discover_pools` LLM tool (token-bloat guard, default = `page_size`); `getTopCandidates` passes `returnLimit:null` (uncapped) so its pre-rank sees the full gate-passed universe. Set `broadDiscoveryEnabled=false` → legacy full-server-filter behavior (fully reversible). Tests: `scripts/test-broad-discovery.js` (38 assertions).

**Market-regime gate (Cassiopeia — STOP BLEED T3):** roadmap step-1 fix for the T3 bleed (-$4.67). ROOT CAUSE: memecoin narrow-range pools deployed into a FALLING market get stopped out repeatedly (price drifts down out of the active bin → cut at stop → repeat); a single-side-SOL narrow position has an ASYMMETRIC payoff in a downtrend (limited bounce upside, full bleed if it keeps falling). FIX: PAUSE memecoin deploys while the broad market trends down. **This ADDS a pause (stricter) — it does NOT loosen any other gate.** Detection: `detectMarketRegime()` (exported) reads SOL 24h % change (SOL = beta of the Solana memecoin complex) via the boss-report price chain — CoinGecko `simple/price?include_24hr_change=true` (no history store), Jupiter v3 `priceChange24h` fallback. `classifyRegime(pct, s)` (pure, exported): `pct ≤ regimeDowntrendThresholdPct` (-5) → `DOWNTREND`; `pct ≥ regimeUptrendThresholdPct` (+5) → `UPTREND`; else `NEUTRAL`. Regime cached 10 min (one fetch per cycle, never per-pool). Gate `marketRegimeGateRejectReason(pool, regimeResult, s)` (pure, exported) fires in `getTopCandidates()` BEFORE all enrichment + judge (paused pool costs nothing): pauses ONLY when `regime === DOWNTREND` AND `isMemecoinNarrowProfile(pool)`. **CONDITIONAL / exempt-bluechip-ready:** blue-chip base mints (`BLUECHIP_BASE_MINTS` = wSOL/USDC/USDT) are EXEMPT (symmetric payoff is fine in a downtrend) — Phase 1 bluechip mode is a one-line allow-list edit; for now the funnel is all memecoins so it applies to every deploy. **FAIL-SAFE (anti-pattern #2):** `classifyRegime` uses `strictNumeric` (not `numeric` — `Number(null)===0` would fabricate a flat 0% reading); a missing/non-finite 24h change → `NEUTRAL` (deploy as legacy), NEVER a false `DOWNTREND` and NEVER a blind freeze. So a price-source outage cannot pause deploys; the gate only ever pauses on a positively-measured downtrend. **ANTI-DORMANCY:** fires only on a CONFIRMED downtrend and releases the moment SOL recovers above the threshold next cycle (10-min cache). Reject reason: `market_regime_downtrend_memecoin_paused`. Config: `marketRegimeGateEnabled` (true), `regimeDowntrendThresholdPct` (-5), `regimeUptrendThresholdPct` (+5), all reloadable. Tests: `scripts/test-market-regime-gate.js` (35 assertions).
